...
if($_GET['pluginop'] == 'add' && submitcheck('adduser')) {
	...
	DB::query("INSERT INTO ".DB::table('xunren_user')." (uid, xname, xuser, xdate, xserver, xgroup, xfriend, type, xtext) VALUES ('$_G[uid]', '$xname', '$xuser', '$xdate', '$xserver', '$xgroup', '$xfriend', '1', '$text')");
	showmessage('添加成功!', 'home.php?mod=spacecp&ac=plugin&id=xunren:bangding');
	...
	showmessage('更新成功!', 'home.php?mod=spacecp&ac=plugin&id=xunren:bangding');
}



$repeatusers = array();
foreach(C::t('#xunren#xunren_user')->fetch_all_by_uid($_G['uid']) as $xunren_user) {
